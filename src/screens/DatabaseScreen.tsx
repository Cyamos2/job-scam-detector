...
import { View, Text, StyleSheet, FlatList, Pressable, Image, Modal, TextInput, Alert, ActivityIndicator, ScrollView } from "react-native";
...
export default function DatabaseScreen({ navigation }: Props) {
  ...
  const pickScreenshot = async () => {
    ...
    const res = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 1, selectionLimit: 1 });
    if (res.canceled) return;
    const asset = res.assets?.[0];
    if (asset?.uri) setImageUri(asset.uri);
  };

  const resetForm = () => { setCompany(""); setInput(""); setNotes(""); setImageUri(null); };
  ...
  return (
    <View style={[{ flex: 1 }, bg]}>
      ...
      <FlatList<SavedAnalysis>
        ...
        contentContainerStyle={list.length === 0 ? [styles.center, bg] : { padding: 12 }}
        ListEmptyComponent={
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.cardInfo, text]}>Score {item.score} — {item.verdict} risk</Text>
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalCard, card]}>
            ...
            <ScrollView contentContainerStyle={{ gap: 10 }}>
              <TextInput style={[styles.input, card, text]} placeholder="Company or title (optional)" placeholderTextColor={muted.color as string} value={company} onChangeText={setCompany} />
              <TextInput style={[styles.input, card, text, { height: 120, textAlignVertical: "top" }]} placeholder="Paste job text or a link (https://…)" placeholderTextColor={muted.color as string} value={input} onChangeText={setInput} multiline />
              <TextInput style={[styles.input, card, text, { height: 80, textAlignVertical: "top" }]} placeholder="Notes (optional, not used for scoring)" placeholderTextColor={muted.color as string} value={notes} onChangeText={setNotes} multiline />
              ...
              <View style={{ flexDirection: "row", justifyContent: "flex-end", gap: 10 }}>
                <Pressable onPress={() => { setShowForm(false); resetForm(); }} style={[styles.cancelBtn, card]}>
...
const styles = StyleSheet.create({
  ...
  modalCard: { padding: 16, borderTopLeftRadius: 16, borderTopRightRadius: 16, gap: 12, maxHeight: "85%", borderWidth: 1 },
